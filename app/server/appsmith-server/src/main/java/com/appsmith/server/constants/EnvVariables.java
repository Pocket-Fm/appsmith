package com.appsmith.server.constants;

public enum EnvVariables {
    APPSMITH_INSTANCE_NAME,
    APPSMITH_DB_URL,
    APPSMITH_REDIS_URL,
    APPSMITH_MAIL_ENABLED,
    APPSMITH_MAIL_FROM,
    APPSMITH_REPLY_TO,
    APPSMITH_MAIL_HOST,
    APPSMITH_MAIL_PORT,
    APPSMITH_MAIL_SMTP_AUTH,
    APPSMITH_MAIL_USERNAME,
    APPSMITH_MAIL_PASSWORD,
    APPSMITH_MAIL_SMTP_TLS_ENABLED,
    APPSMITH_SIGNUP_ALLOWED_DOMAINS,
    APPSMITH_ADMIN_EMAILS,
    APPSMITH_RECAPTCHA_SITE_KEY,
    APPSMITH_RECAPTCHA_SECRET_KEY,
    APPSMITH_DISABLE_TELEMETRY,
    APPSMITH_OAUTH2_GOOGLE_CLIENT_ID,
    APPSMITH_OAUTH2_GOOGLE_CLIENT_SECRET,
    APPSMITH_OAUTH2_GITHUB_CLIENT_ID,
    APPSMITH_OAUTH2_GITHUB_CLIENT_SECRET,
    // PocketFM CE fork: OIDC config variables (unlocked from EE)
    APPSMITH_OAUTH2_OIDC_CLIENT_ID,
    APPSMITH_OAUTH2_OIDC_CLIENT_SECRET,
    APPSMITH_OAUTH2_OIDC_AUTHORIZATION_URL,
    APPSMITH_OAUTH2_OIDC_TOKEN_URL,
    APPSMITH_OAUTH2_OIDC_USERINFO_URL,
    APPSMITH_OAUTH2_OIDC_JWKS_URL,
    APPSMITH_OAUTH2_OIDC_SCOPES,
    APPSMITH_OAUTH2_OIDC_USERNAME_ATTRIBUTE,
    // PocketFM CE fork: SAML config variables (unlocked from EE)
    APPSMITH_SAML_ENTITY_ID,
    APPSMITH_SAML_METADATA_URL,
    APPSMITH_SAML_SSO_URL,
    APPSMITH_SAML_PUB_CERT,
    APPSMITH_CUSTOM_DOMAIN,
    APPSMITH_ALLOWED_FRAME_ANCESTORS,
    APPSMITH_DISABLE_IFRAME_WIDGET_SANDBOX,
    APPSMITH_NEW_RELIC_ACCOUNT_ENABLE,
    APPSMITH_VERBOSE_LOGGING_ENABLED
}
